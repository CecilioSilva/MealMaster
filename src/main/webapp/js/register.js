...
function register() {
  ...
  if (jsonRequestBody.name.length < 3 || jsonRequestBody.name.length > 40) {
    errorMsg.innerHTML = "Name should be between 3 and 40 characters";
    return;
  }
  ...
  fetch("api/auth/register", {
    ...
    .then((res) => {
      if (res.ok) {
        ...
        window.location = "/login.html?status=Account created successfully!";
      } else {
